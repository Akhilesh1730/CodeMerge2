...
class LeavesApprovalPendingAdapter(private val dataList: List<LeavesApproval>)
    ...
    private var onClickListener: LeavesApprovalPendingAdapter.OnClickListener? = null
    ...
    interface OnClickListener {
        fun onApproveClicked(model: LeavesApproval)
        fun onRejectedClicked(model: LeavesApproval)
    }
    ...
    override fun onBindViewHolder(holder: LeavesApprovalPendingViewHolder, position: Int) {
        ...
        holder.buttonApprove.setOnClickListener {
           var remarkText = holder.editTextLeavesApprovalRemark.text.toString()
            model.remark = if (remarkText.isNullOrEmpty()) "" else remarkText
            onClickListener!!.onApproveClicked(model)
        }
        holder.buttonReject.setOnClickListener {
            var remarkText = holder.editTextLeavesApprovalRemark.text.toString()
            model.remark = if (remarkText.isNullOrEmpty()) "" else remarkText
            onClickListener!!.onRejectedClicked(model)
        }
    }
}